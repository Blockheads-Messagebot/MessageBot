...
var MacChatParser = (function () {
    ...
    function MacChatParser(online) {
        ...
        this.handleJoin = function (name, ip) {
            if (!_this.online.includes(name)) {
                _this.online.push(name);
            }
            _this.messages.push({ type: chat_1.ChatType.join, name: name, ip: ip });
        };
        /**
         ...
         */
        this.handleLeave = function (name) {
            if (_this.online.includes(name)) {
                _this.online.splice(_this.online.indexOf(name), 1);
                _this.messages.push({ type: chat_1.ChatType.leave, name: name });
            }
        };
        /**
         ...
         */
        this.handleChat = function (name, message) {
            if (name == 'SERVER') {
                // Server chat must be ignored to avoid an infinite loop.
                return;
            }
            _this.messages.push({ type: chat_1.ChatType.message, name: name, message: message });
            if (message.startsWith('/') && !message.startsWith('/ ')) {
                ...
                _this.messages.push({ type: chat_1.ChatType.command, name: name, command: command, args: args });
            }
        };
        /**
        ...
        this.getUsername = function (message) {
            for (var i = 18; i > 4; i--) {
                ...
                if (_this.online.includes(possibleName) || possibleName == 'SERVER') {
                    return possibleName;
                }
            }
