...
var TabManager = (function () {
    ...
    function TabManager(navigationRoot, contentRoot) {
         ...
         */
        this.addTab = function (text, groupName) {
            var tabName = 'tab_' + _this.tabUID++;
            ...
            var navParent;
            if (groupName) {
                navParent = _this.navigationRoot.querySelector("[data-tab-group=" + groupName + "]");
                if (!navParent) {
                    throw new Error("Tab group " + groupName + " does not exist.");
                }
            }
            else {
                navParent = _this.navigationRoot;
            }
            navParent.appendChild(tab);
            _this.contentRoot.appendChild(content);
            return content;
        };
        /**
         ...
         */
        this.removeTab = function (content) {
            var nav = _this.navigationRoot.querySelector(".nav-item[data-tab-name=" + content.dataset.tabName + "]");
            if (nav) {
                nav.remove();
                content.remove();
                return true;
         ...
         */
        this.addTabGroup = function (text, groupName, parent) {
            if (_this.navigationRoot.querySelector("[data-tab-group=\"" + groupName + "\"]")) {
            ...
            var parentNav;
            if (parent) {
                parentNav = _this.navigationRoot.querySelector("[data-tab-group=\"" + parent + "\"]");
            }
            else {
                parentNav = _this.navigationRoot;
            }
            if (parentNav) {
         ...
         */
        this.removeTabGroup = function (groupName) {
            var group = _this.navigationRoot.querySelector("[data-tab-group=\"" + groupName + "\"]");
            if (!group) {
                return false;
            }
            try {
                for (var _a = __values(group.querySelectorAll('span')), _b = _a.next(); !_b.done; _b = _a.next()) {
                    var item = _b.value;
                    _this.contentRoot.querySelector("[data-tab-name=\"" + item.dataset.tabName + "\"]").remove();
                }
            }
